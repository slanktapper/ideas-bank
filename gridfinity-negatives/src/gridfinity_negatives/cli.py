...
def cmd_gauge(a: argparse.Namespace) -> int:
    """Print a ladder of gap gauges, skipping lengths already owned."""
    deltas = tuple(float(d) for d in a.deltas) if a.deltas else STANDARD_DELTAS
    want = ladder(a.gap, deltas)
    have, need = check_ladder(a.gap, a.library, deltas)
    lib = load_library(a.library)

    print(f"Nominal gap : {a.gap:g} mm")
    print(f"Ladder      : " + ", ".join(f"{L:g}" for L in want) + " mm"
          + ("" if a.deltas else "   (standard -3 -2 -1 0 +1)"))
    if have:
        print(f"Already own : " + ", ".join(f"{L:g}" for L in have) + " mm"
              " -- reuse these, do not print them again")
        for L in have:
            if lib.get(L):
                print(f"                {L:g} mm: {lib[L]}")
    if not need:
        print()
        print("Nothing to print. The whole ladder is already in the library.")
        return 0

    print(f"To print    : " + ", ".join(f"{L:g}" for L in need) + " mm")

    only = None if a.all else set(need)
    try:
        paths = export_set(a.gap, a.out, a.name, deltas, only=only)
    except ValueError as e:
        print(f"error: {e}", file=sys.stderr)
        return 1

    g = make_gauge(a.gap)
    print(f"Each gauge  : {g.vals()[0].BoundingBox().xlen:.0f} x (length) x "
          f"{g.vals()[0].BoundingBox().zlen:.0f} mm, "
    ...
    if a.register:
        for L in need:
            lib[L] = a.register
        save_library(lib, a.library)
        print()
        print(f"recorded {len(need)} new length(s) in {a.library}")
    else:
        print()
        print(f"Once printed, record them:  --register \"<what they were for>\"")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    ga.add_argument("--deltas", nargs="*", default=None,
                    help="offsets from nominal (default: the standard -3 -2 -1 0 +1)")
    ga.add_argument("--library", default=LIBRARY_FILE,
                    help="file listing gauges that physically exist")
    ga.add_argument("--all", action="store_true",
                    help="export the whole ladder, including lengths already owned")
    ga.add_argument("--register", metavar="NOTE",
                    help="record the new lengths in the library after printing")
